...
def precompute_rotary_emb(dim, max_positions):
    ...
    ### START CODE HERE
    half_dim = dim // 2
    idx = torch.arange(half_dim, dtype=torch.float)
    theta = 1.0 / (10000 ** (2 * idx / dim))
...
def apply_rotary_emb(x, rope_cache):
    ...
    cache_complex = torch.view_as_complex(cache)

    out_complex = x_complex * cache_complex

    out_real = torch.view_as_real(out_complex)
    rotated_x = out_real.reshape(*x.shape)
    ### END CODE HERE
    return rotated_x
